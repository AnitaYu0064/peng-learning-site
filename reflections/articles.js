window.REFLECTION_ARTICLES = [
  {
    date: "2026-03-31",
    title: "《被討厭的勇氣》我的觀點？",
    tags: ["書籍反思", "主觀世界", "目的論", "責任與選擇"],
    description:
      "聚焦分析《被討厭的勇氣》的核心論點，從主觀世界、目的論、理解與改變、個人責任到自由選擇的條件，逐步整理出對這本書的質疑與判斷。",
    url: "courage-my-view.html",
    status: "階段完成"
  },
  {
    date: "2026-04-04",
    title: "自由與責任，是否忽略了能動性條件？",
    tags: ["阿德勒心理學", "能動性", "自由", "責任邊界"],
    description:
      "從能動性（agency）的角度重新檢視《被討厭的勇氣》，思考這套理論是否忽略不同心理狀態與現實條件差異，以及過度套用後可能帶來的壓力與誤用。",
    url: "courage-agency-responsibility.html",
    status: "階段完成"
  },
  {
    date: "2026-04-21",
    title: "現行教育制度真的適合所有學生嗎？",
    tags: ["教育制度", "學習動力", "個體差異", "好奇心"],
    description:
      "從任務、分數與升學邏輯切入，分析現行教育制度如何忽略學生差異、壓縮好奇心，並思考教育真正應該優先保護的是什麼。",
    url: "education-system-fit-all-students.html",
    status: "階段完成"
  },
  {
    date: "2026-05-12",
    title: "我所理解的哲學",
    tags: ["哲學理解", "思辨能力", "價值觀", "自我形成"],
    description:
      "說明哲學對頌哲而言不是背誦哲學史，而是一種持續思考、懷疑、分析並形成自己觀點的方式，也整理哲學如何真正進入他的生活與判斷。",
    url: "what-philosophy-means-to-me.html",
    status: "階段完成"
  },
  {
    date: "2026-06-07",
    title: "如何區分愛、依附、承諾、責任與恐懼？",
    tags: ["心理學", "親密關係", "依附理論", "愛情三角理論", "自我分化"],
    description:
      "從心理學觀點分析親密關係中的愛，並嘗試區分愛、依附、承諾、責任、恐懼與習慣之間的差異。",
    url: "love-attachment-commitment-responsibility-fear.html",
    status: "階段完成"
  },
  {
    date: "2026-07-27",
    title: "為什麼偏偏是他？——論愛能否被理性解釋",
    tags: ["哲學思辨", "愛的本質", "理性與感性", "個人觀點"],
    description:
      "從條件、長期相處與優點等常見解釋出發，持續追問人為什麼會愛上某一個特定的人，以及愛是否能被理性完整解釋。",
    url: "why-him-love-rationally-explained.html",
    status: "階段完成"
  },
  {
    date: "2026-07-27",
    title: "《為什麼偏偏是他？》：愛情哲學觀點的形成與修正歷程",
    tags: ["學習歷程", "哲學思辨", "觀點修正", "愛的本質", "研究反思"],
    description:
      "以原本想法、案例觀察、疑問、漏洞與觀點修正為主線，整理頌哲如何從愛與依附的問題出發，逐步形成對條件、優點、特殊性、理性與主觀經驗的看法。",
    url: "love-philosophy-thinking-process.html",
    status: "階段完成"
  }
];
